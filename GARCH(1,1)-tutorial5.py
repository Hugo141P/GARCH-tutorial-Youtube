...
def VaR(theta, dist, alpha):
    VaR_alpha = inverse_cdf(theta, dist, alpha)
    return VaR_alpha

def ES(theta, dist, alpha):
    VaR_alpha = VaR(theta, dist, alpha)

    lower_bound = -100
    ES_alpha = quad(lambda x: x * pdf(theta, dist, x), lower_bound, VaR_alpha)[0] / alpha
    return ES_alpha
...
def GARCH(dist, returns):
    ...
    alpha = 0.05
    VaR_005 = VaR(theta_ML, dist, alpha) * np.sqrt(cond_variance)
    ES_005 = ES(theta_ML, dist, alpha) * np.sqrt(cond_variance)
    plt.plot(returns, label='returns')
    plt.plot(np.sqrt(cond_variance), label='volatility')
    plt.plot(VaR_005, label='5% VaR')
    plt.plot(ES_005, label='5% ES')
    plt.title('returns and volatility from GARCH(1, 1)-{} model'.format(dist))
    plt.legend()
    plt.savefig(f'GARCH(1, 1)-{dist}.png')
    plt.show()
    plt.close()
